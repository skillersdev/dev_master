				...
				<div class="row">
					<div class="col-sm-12 portlet">
						<section class="panel panel-success portlet-item">
							<header class="panel-heading"> Earning Settings </header>
							<section class="panel panel-default">
								<header class="panel-heading bg-light">
									<ul class="nav nav-tabs nav-justified">
										<li class="active"><a href="#home" data-toggle="tab">Create Earning Level</a></li>
										<li><a href="#profile" data-toggle="tab">Update Earning Level</a></li>
									</ul>
								</header>
								<div class="panel-body">
									<div class="tab-content">
										<div class="tab-pane active" id="home">
											<div class="panel-body">
												<form action="createearning.php" method="post">
													<div class="form-group">
													<label>Downline Count</label>
													<input type="text" class="form-control" placeholder="Enter Downline Count" name="downline_count">
													</div>
													<div class="form-group">
													<label>Maximum Earning Amount</label>
													<input type="text" class="form-control" placeholder="Maximum Earning Amount" name="earning_amt">
													</div>
													<button type="submit" class="btn btn-lg btn-primary btn-block">Create Earning Level</button>
												</form>
											</div>
										</div>
										<div class="tab-pane" id="profile">
											<form action="updateearning.php" method="post">
												<div class="form-group">
													<label>Select Earning Level : </label>
													<select name="uearning">
														<?php
														$query="SELECT id,downline_count,earning_amt FROM `earning_settings`";
														$result = mysqli_query($con,$query);
														while($row = mysqli_fetch_array($result))
														{
															echo '<option value="'.$row[id].'">Downline('.$row[downline_count].') | Amount('.$row[earning_amt].')</option>';
														}
														?>
													</select>
												</div>
