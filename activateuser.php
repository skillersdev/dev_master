<?php
header( "refresh:5;url=paymentscod.php" );
...
if($c1_count==0)
{
	...
	*/
	...
	if($ref_under_count<2)
	{
		...
		/* karthikeyan code for bonus restriction starts */
		$query=mysqli_query($con,"update affiliateuser set level='".$ref_stage."' where username='".$ref."'");
		$downline_sql=mysqli_query($con,"SELECT COUNT(referedby) As TotalDownline FROM affiliateuser WHERE referedby = '$ref'");
		$downline_row=$downline_sql->fetch_array(MYSQLI_ASSOC);
		$tot_downline=$downline_row['TotalDownline'];
		
		$earning_sql=mysqli_query($con,"SELECT downline_count, earning_amt FROM earning_settings WHERE active='1'");
		while ($earning_row = mysqli_fetch_array($earning_sql, MYSQL_ASSOC)) {
			
			if (($tot_downline >= $earning_row['downline_count']) && ($tot_amt < $earning_row['earning_amt'])){
				//check and and apply here
				$query=mysqli_query($con,"update affiliateuser set tamount='".$tot_amt."' where username='".$ref."'");
				//till this
				break;
			}
		}		
		...
		while($ref_bonus_user !=""){
			...
			if($ref_bonus_user2 !=""){
				...
				/* karthikeyan code for bonus restriction starts */
				$downline_sql1=mysqli_query($con,"SELECT COUNT(referedby) As TotalDownline FROM affiliateuser WHERE referedby = '$ref_bonus_user2'");
				$downline_row1=$downline_sql1->fetch_array(MYSQLI_ASSOC);
				$tot_downline1=$downline_row1['TotalDownline'];

				$earning_sql1=mysqli_query($con,"SELECT downline_count, earning_amt FROM earning_settings WHERE active='1'");
	...
	elseif($ref_under_count>=2){
	 ...
	 //    exit;
		...
		$downline_sql=mysqli_query($con,"SELECT COUNT(referedby) As TotalDownline FROM affiliateuser WHERE referedby = '$stage1_ref'");
		$downline_row=$downline_sql->fetch_array(MYSQLI_ASSOC);
		$tot_downline=$downline_row['TotalDownline'];
		...
		while ($earning_row = mysqli_fetch_array($earning_sql, MYSQL_ASSOC)) {
			...
			if (($tot_downline >= $earning_row['downline_count']) && ($tot_amt < $earning_row['earning_amt'])){
				//check and and apply here
				$query=mysqli_query($con,"update affiliateuser set tamount='".$tot_amt."' where username='".$stage1_ref."'");
				//till here
			}
		}
		/* karthikeyan code for bonus restriction ends */
			...
			for($i=1;$i<=100;$i++){
				...
				if($current_tot>=$$pup && $current_tot!=0 && $curr_stage ==$i){
					...
					if($upgrade_his==0){
						...
						/* karthikeyan code for bonus restriction starts */
						$downline_sql1=mysqli_query($con,"SELECT COUNT(referedby) As TotalDownline FROM affiliateuser WHERE referedby = '$stage1_ref'");
						$downline_row1=$downline_sql1->fetch_array(MYSQLI_ASSOC);
						$tot_downline1=$downline_row1['TotalDownline'];
						...
						while ($earning_row1 = mysqli_fetch_array($earning_sql1, MYSQL_ASSOC)) {
							...
							if (($tot_downline1 >= $earning_row1['downline_count']) && ($tot_amt9 < $earning_row1['earning_amt'])){
								//check and and apply here
	...
	//upgradation process code ends 
		...
		while($ref_bonus_user !=""){
			...
			if($ref_bonus_user2 !=""){
				...
				/* karthikeyan code for bonus restriction starts */
				$downline_sql=mysqli_query($con,"SELECT COUNT(referedby) As TotalDownline FROM affiliateuser WHERE referedby = '$ref_bonus_user2'");
				$downline_row=$downline_sql->fetch_array(MYSQLI_ASSOC);
				$tot_downline=$downline_row['TotalDownline'];
...
if($result){
echo "<center>Profile Activated<br/>Redirecting in 5 seconds...</center>";
}
else
{
echo "<center>Action could not be performed, Something went wrong. Check back again<br/>Redirecting in 2 seconds...</center>";
}