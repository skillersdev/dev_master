...
$upid = mysqli_real_escape_string($con,$_POST['uearning']);
?>
<!DOCTYPE html>
            ...
            <div class="row">
              ...
              <div class="col-sm-12 portlet">
                <section class="panel panel-success portlet-item">
                  <header class="panel-heading">Earning Level Details</header>
                  <section class="panel panel-default">
                      <?php 
					  
					  $query="SELECT id,downline_count,earning_amt,active FROM `earning_settings` where id=$upid"; 
...
while($row = mysqli_fetch_array($result))
{
	...
	$id="$row[id]";
	$downline_count="$row[downline_count]";
	...
	$active="$row[active]";
                  ...
                  <div class="panel-body">
                    <div class="tab-content">
					  ...
					  <div class="panel-body">
                    <form action="updateearningsettings.php" method="post">
					 <input type="hidden" value="<?php print $id ?>" name="id">
					<div class="form-group">
                        <label>Downline Count</label>
						<input type="text" class="form-control" placeholder="Enter Downline Count" name="downline_count" value="<?php echo $downline_count;?>">
                      </div>
                      <div class="form-group">
                        <label>Maximum Earning Amount</label>
                        <input type="text" class="form-control" placeholder="Maximum Earning Amount" name="earning_amt" value="<?php echo $earning_amt;?>">
                      </div>
                      <div class="form-group">
                        <label>Status</label><br/>
                        <select name="active">
							<option value="1" <?php echo ($active == '1') ? 'selected="selected"' : '';?>>Active</option>
							<option value="0" <?php echo ($active == '0') ? 'selected="selected"' : '';?>>Deactive</option>
						</select>
                      </div>
                     <button type="submit" class="btn btn-lg btn-primary btn-block">Save Details</button>
                    </form>
                  </div>
